...
adjust_all(integer doOffset, vector amt) {
    ...
    for (ix = 0; ix < PosCount; ++ix) {
        ...
        while (ballix < Ballcount) {
            string stuff = adjust(doOffset, (vector)llList2String(parms, 2*ballix), (vector)llList2String(parms, 2*ballix+1), amt);
            data += " " + stuff;
            ++ballix;
        }
        store_pose(data, ix);
...
check_pose(string name) {
    ...
    if (name == "CHECK2") {
        ...
        for (ix = 0; ix < llGetListLength(Poses); ++ix) {
            posename = llList2String(Poses, ix);
            if (get_pose_data(posename) != "") {
                if (posename != "default" && posename != "stand") {
                    llOwnerSay("No .MENUITEMS* entry for '" + posename + "'.");
                }
            }
        }
        llOwnerSay("Checks complete, resetting.");
        llMessageLinked(LinkScope_i, -100, llDumpList2String([llGetScriptName(), "~props"], "|"), (key)"#RESET");
        return;
    }
...
default_state_exit()
{
    ...
    llMessageLinked(LinkScope_i, -100, llDumpList2String(["~props", "~sequencer"], "|"), (key)"#RESET");
...
state on {
    state_entry() {
        llMessageLinked(LinkScope_i, 2, "OK", (key)""); //msg to menu, in case it's waiting for loading
        announce();
    }
    ...
    link_message(integer from, integer num, string str, key dkey) {
        ...
        if (num == 0 && str == "POSEB") {
            ...
            if (name == "CHECK1") {
                Checking = TRUE;
                    ...
                    llMessageLinked(LinkScope_i, 0, "POSEPOS", (key)get_pose_data((string)dkey));  // to ~pos
            }
            return;
        }
        ...
        if (str == "OK?") {                                 //question from menu, before loading menu
            ...
            integer ix;
            for (ix = 0; ix < PosCount; ++ix) {
                string posename = llList2String(Poses, ix);
                llOwnerSay("{" + posename + "} " + get_pose_data(posename));
            }
            ...
            llMessageLinked(LinkScope_i, 0, "AGAIN", (key)"");
