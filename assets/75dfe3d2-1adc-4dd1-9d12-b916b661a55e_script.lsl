...
vector startColor = <1.0, 0.6, 0.6>;     // RGB color, black<0,0,0> to white<1,1,1>(*)
vector   endColor = <1.0, 0.6, 0.6>;     // 
float  startAlpha = 1;         // 0.0 to 1.0(*), lower = more transparent
float    endAlpha = 0.1;         // 
vector  startSize = <0.2,0.2,0.2>; // <0.04,0.04,0>(min) to <10,10,0>(max>, <1,1,0>(*)
vector    endSize = <0.2,0.2,0.2>; // (Z part of vector is discarded)  
string    texture = "leaf";          // Texture used for particles. Texture must be in prim's inventory.

// SECTION TWO: FLOW -- These settings affect how Many, how Quickly, and for how Long particles are created.
//     Note, 
integer count = 4;    // Number of particles created per burst, 1(*) to 4096
float    rate = 2.5;   // Delay between bursts of new particles, 0.0 to 60, 0.1(*)
...
float      radius = 3;      // 0.0(default) to 64?  Distance from Emitter where new particles are created.
...
vector      omega = <0,0,-1>; // How much to rotate the emitter around the <X,Y,Z> axises. <0,0,0>(*)
...
float       minSpeed = 0.9;     // 0.01 to ? Min speed each particle is spit out at, 1.0(*)
float       maxSpeed = 2.5;     // 0.01 to ? Max speed each particle is spit out at, 1.0(*)
vector          push = <0,0,-1>; // Continuous force pushed on particles, use small settings for long lived particles
