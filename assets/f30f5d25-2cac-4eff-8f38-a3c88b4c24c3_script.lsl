...
/|/ This version was modified by Snoppy Pfeffer to work in OpenSim worlds.  01/06/10
/|/ Requires Opensim SVN #9110 or higher
...
/|/ Snoopy Pfeffer, 10/08: Ported to run with in OpenSim worlds.
...
string defaultNoteCard = "My Anims";
...
integer standTime;                          /|/ How long before flipping stand animations
...
loadNoteCard() {
    ...
    /|/ llMinEventDelay( 0 );  /|/ caused notecard not to load
...
default {
    state_entry() {
        ...
        standTime = standTimeDefault;       /|/ How long before flipping stand animations
...
/|/    }
    ...
    {
        if ((change & CHANGED_ANIMATION) && animOverrideOn) {  /|/ requires Opensim SVN #9110 or later
            if ( checkAndOverride() ) {
...
//program_version Phoenix Viewer 1.5.2.1050 - Avatar Test
//mono
...
default {
    ...
    changed(integer change)
    {
        if ((change & CHANGED_ANIMATION) && animOverrideOn) {  
            if ( checkAndOverride() ) {
